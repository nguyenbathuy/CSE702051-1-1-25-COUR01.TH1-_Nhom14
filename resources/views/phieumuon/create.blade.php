...
@section('content')
    <div class="container-fluid">
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Tạo Phiếu Mượn Mới</h1>
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Biểu Mẫu Tạo Phiếu Mượn</h6>
            </div>
            <div class="card-body">
                <form action="{{ route('phieumuon.store') }}" method="POST">
                    @csrf
                    <div class="mb-3">
                        ...
                        <select class="form-control @error('book_item_id') is-invalid @enderror" id="book_item_id" name="book_item_id"
                            ...
                            @foreach($bookItems as $bookItem)
                                <option value="{{ $bookItem->id }}" {{ old('book_item_id') == $bookItem->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('borrowed_date') is-invalid @enderror"
                            id="borrowed_date" name="borrowed_date" value="{{ old('borrowed_date') }}" required>
                        @error('borrowed_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
